...
public:
    ...
    void set_gamma_s(real gs);
    real get_gamma_s(void);

    void set_gamma_c(real gc);
    real get_gamma_c(void);
...
protected:
    ...
    MatrixXi mPntIdxKnlRow; // A 2D index matrix. Each element is the original row index of a single kernel center.
    MatrixXi mPntIdxKnlCol; // A 2D index matrix. Each element is the original col index of a single kernel center.

    MatrixXd mDistanceMap; // A 2D map. Each element of this map records its distance from the center of the window.
    MatrixXd mWsMap;

    MatrixXd mPntDistKnl; // A small 2D matrix. Each element of this matrix is the referenced distance from a kernel center to the window center.
