...
__global__
void exponent(int n, const CRReal_t* input, CRReal_t* output, int rows, int cols, int cStep, int w = 39)
{
    ...
    const auto m = Eigen::Map< 
        const Eigen::Matrix< CRReal_t, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor >, 
        Eigen::Unaligned, 
...
int crExponent(const CRReal_t* input, int rows, int cols, int cStep, CRReal_t* output)
{
    ...
    CRReal_t* dmIn  = NULL; // Device memeory pointer.
    CRReal_t* dmOut = NULL; // Device memeory pointer.

    // Allocate and copy the memory from CPU to GPU.
    const int size = rows*cols*cStep;
    cudaMallocManaged(&dmIn,  size*sizeof( CRReal_t ));
    cudaMallocManaged(&dmOut, size*sizeof( CRReal_t ));

    printf("dmIn is of size %d.\n", size*sizeof(CRReal_t));
